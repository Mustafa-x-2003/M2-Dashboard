...
export default function AddProductsHeader({type , title , desc , RightStatus}) {
      ...
      <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-cyan-600 dark:text-cyan-300 transition-colors duration-300">{type}</p>
              <h1 className="mt-1 text-4xl font-black text-slate-900 dark:text-white transition-colors duration-300">{title}</h1>
            </div>
          </div>

          <p className="mt-4 max-w-3xl text-slate-600 dark:text-slate-300 transition-colors duration-300">
            {desc}
          </p>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white/50 p-4 shadow-lg backdrop-blur shrink-0 dark:border-white/10 dark:bg-white/5 dark:shadow-xl transition-all duration-300">
          <p className="text-xs uppercase tracking-[0.3em] text-cyan-600 dark:text-cyan-300 transition-colors duration-300">{RightStatus.title}</p>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-200 transition-colors duration-300">{RightStatus.desc}</p>
        </div>
      </div>
    </div>
  );
